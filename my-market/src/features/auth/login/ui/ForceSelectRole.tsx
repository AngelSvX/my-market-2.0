...
import { force_select_role, googleLogin } from '../model/thunks';
import { getRoleId, roleWasSelected } from "../model/slice";
import { useNavigate } from "react-router";

function ForceSelectRole() {
  const [selectedRole, setSelectedRole] = useState<number | null>(null);

  const dispatch = useDispatch<AppDispatch>()

  const { userData, loading, getRole, roleSelected, google_token } = useSelector((state: RootState) => state.login)
      ...
      <div className="bg-white w-full max-w-xl rounded-3xl shadow-md p-10 space-y-8 border border-gray-100">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <button
            onClick={() => {
              setSelectedRole(2)
              dispatch(getRoleId({ id: Number(userData?.id), role_id: 2 }))
            }}
            className={`
          ...
          <button
            onClick={() => {
              setSelectedRole(3)
              dispatch(getRoleId({ id: Number(userData?.id), role_id: 3 }))
            }}
            className={`
              group border rounded-2xl p-6 transition-all flex flex-col items-center gap-3
              ${selectedRole === 3
            ...
            loading ?
              ...
              (
                <button
                  ...
                  onClick={() => {
                    if (getRole) {
                      dispatch(force_select_role(getRole))
                      dispatch(roleWasSelected(true))
                    }
                  }}
                >
                  Continuar
...
export default ForceSelectRole;
